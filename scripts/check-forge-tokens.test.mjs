...
test('findForbiddenTokens returns no violations for clean source', () => {
  const dir = mkdtempSync(join(tmpdir(), 'dc-guard-'))
...
test('findForbiddenTokens flags a forbidden token with file and line', () => {
  const dir = mkdtempSync(join(tmpdir(), 'dc-guard-'))
...
test('findForbiddenTokens ignores non-.ts files', () => {
  const dir = mkdtempSync(join(tmpdir(), 'dc-guard-'))
  writeFileSync(join(dir, 'notes.md'), 'this mentions forge and license freely\n')
  const violations = findForbiddenTokens(dir)
  assert.deepEqual(violations, [])
  rmSync(dir, { recursive: true, force: true })
})
